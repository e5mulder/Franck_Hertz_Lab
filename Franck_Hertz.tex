...
In the experiment, a beam of electrons is sent through a cloud of atoms.
Since atoms can only gain energy in an amount equal to an allowed transition between energy levels, only electrons with energies equal to these transition energies can transfer their energy to an atom. 
...
The loss in energy prevents the electrons from overcoming the retarding voltage at the anode. 
...
Then was done so we could select an optimum temperature at which to take data using a lock-in amplifier. 
...
\section{Analysis}
Since we only saw two rough dips in our neon data, we did no further analysis on it. Our mercury data was imported into Igor Pro. Our analysis was performed on data at 204\degree C. 
...
At this point, we decided to estimate our minimum by eye as well as the uncertainty for our both sets of data. Then we calculated the spacing between successive minima (Table ~\ref{minimaSpacing} and Fig. \ref{nomineye}). 
