...
The connection between temperature and rate of increase is the mean free path of the gas.  At lower temperatures, the gas is less dense.  This means that an electron travels further on average between collisions with atoms - the ``mean free path'' is this distance. A longer path between collisions means the atom has more time to gain energy as the accelerating voltage propels it through the gas, and so it has more ``extra'' energy between collisions.  Therefore the electrons which reach the anode have more ``extra'' energy as well, which causes an increase in the entire dataset - a background. The magnitude of the background is temperature dependent. 
...
We can improve the accuracy and precision our experiment in a few ways. One would be increasing the control for the accelerating voltage, so smaller increments could be made, increasing the number of data points. This would increase the definition of our minima and make it easier to determine their locations. Another would be finding a new way to compute minima locations more quickly and with more reasonable error using a data analysis program. 
